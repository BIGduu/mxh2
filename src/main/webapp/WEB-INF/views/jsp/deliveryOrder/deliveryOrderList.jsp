        	...
        	<c:forEach items="${deliveryOrderList}" var="deliveryOrder">
	            <tr>
	                ...
	                <td class="text-center">
	                    <div class="btn-group">
	                     	<button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="确认出库" onclick="confirmLibrary('${deliveryOrder.id}')">确认出库</button>
	                    </div>
	                </td>
	            </tr>
            </c:forEach>
...
function goList(page)
{
	...
	goMenu("/admin/deliveryOrder/deliveryOrderList", page, {
		selectId : selectId,
		searchName : searchName
	});
}

function confirmLibrary(deliveryOrderId) {
		ajax({
			type: "POST",
	        dataType: "html",
	        url: "/admin/deliveryOrder/confirmLibrary?deliveryOrderId="+ deliveryOrderId,
			timeout:5000,
			error: function() 
			{
				tipFail("网络异常，请稍后再试");
