    ...
    <div class="row items-push">
        <div class="col-sm-7">
            <h1 class="page-heading">
                <small><a onclick="goMain()">首页</a> > 客户材料部对账订单</small>
            </h1>
        </div>
    </div>
</div>
...
{
	goMenu("/admin/order/custMaterialsOrderListCheck", page, $("#searchForm").serialize());
}

function exportExcel(){
	var exportFormObj = document.getElementById("exportForm");
	exportFormObj.action="/mxh/admin/order/exportOrderCheckList?type=4&"+$("#searchForm").serialize();
...
function orderMenuList(orderId, orderName) {
	ajax({
		type: "POST",
        dataType: "html",
        url: "/admin/order/custMaterialsOrderListCheck?page=${page}&orderId="+orderId,
		timeout:5000,
		data: {
			orderName : orderName
		},
...
function back(orderId) {
	layer.prompt({title: '请填写驳回原因'}, function(pass, index){
		layer.close(index);
		index = layer.load(1, {
			shade: [0.5,'#000'] //0.1透明度的白色背景
		});
