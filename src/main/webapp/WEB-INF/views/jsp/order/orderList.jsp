    ...
    <div class="row items-push">
        <div class="col-sm-7">
            <h1 class="page-heading">
                <small><a onclick="goMain()">首页</a> > 订单列表</small>
            </h1>
        </div>
    </div>
</div>
...
<div class="content">
	...
	<form class="form-horizontal" autocomplete="off" action="" method="post" onsubmit="return false;" id="searchForm">
		<div class="row">
			...
			<div class="common-query-unit" id="searchTitleDiv">
				<div class="form-group">
					<div style="line-height: 25px;display: inline-block;padding-left:15px;" id="managerIdTitle">司机：</div>
			    	<div style="width: 65%;display: inline-block;">
		    			<select class="js-select2" name="deliveryId" style="width:90%;margin-top: -2px;" data-placeholder="">
				    	...
				    	<div style="width: 65%;display: inline-block;">
				    		<select class="js-select2" name="state" style="width:90%;margin-top: -2px;" data-placeholder="">
						      	<option value="" selected="selected">请选择：</option>
						   		...
						   		<option value="5" >已出库待司机送达</option>
						   		<option value="11" >待文员处理步梯上楼费</option>
	                    ...
	                    <div class="btn-group">
							...
							<c:choose>
			   					<c:when test="${order.state == 1 }"> 
			   						<button class="btn btn-xs btn-default" type="button"  title="通过" onclick="confirmExamine(${order.id})">通过</button>
									<button class="btn btn-xs btn-default" type="button"  title="拒绝" onclick="confirmDelete(${order.id})">拒绝</button>
				   				</c:when>
			   					<c:when test="${order.state == 2 }"> 
			   						<button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="分配订单" onclick="orderAllocation('${order.id}')">分配订单</button>
				   				</c:when>
			   					<c:when test="${order.state == 0 }"> 
			   						<button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="异常已处理" onclick="orderAbnormal('${order.id}')">异常已处理</button>
				   				</c:when>
							</c:choose>
							...
							<c:if test="${sessionScope.admin.roleId == 2}">
								<button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="备注" onclick="orderRemark('${order.id}')">备注</button>
							</c:if>
	                    </div >
	                </td>
	            </tr>
...
{
	goMenu("/admin/order/orderList", page, $("#searchForm").serialize());
}
...
function orderMenuList(orderId, orderName) {
	ajax({
		type: "POST",
        dataType: "html",
        url: "/admin/order/orderList?page=${page}&orderId="+orderId,
		timeout:5000,
		data: {
			orderName : orderName
		},
...
function orderDelete(orderId) {
	ajax({
		type: "POST",
        dataType: "html",
        url: "/admin/order/delete?orderId="+orderId,
		timeout:5000,
		data: {},
		error: function() {
			tipFail("网络异常，请稍后再试");
...
function changeRemarks(orderId,remarks){
	ajax({
		type: "POST",
        dataType: "html",
        url: "/admin/order/orderRemark?orderId="+orderId,
