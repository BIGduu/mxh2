    ...
    <div class="row items-push">
        <div class="col-sm-7">
            <h1 class="page-heading">
                <small><a onclick="goMain()">首页</a> > 对账订单</small>
            </h1>
        </div>
    </div>
</div>
        ...
        <div class="row">
            <div class="common-query-unit" id="searchTitleDiv">
                <div class="form-group">
                    <div style="line-height: 25px;display: inline-block;padding-left:15px;" id="orderCodeTitle">订单编号：
                    </div>
                    <div style="width: 65%;display: inline-block;">
                        <input class="form-control" style="height:28px;width: 87%;" type="text" id="orderCode"
            ...
            <div class="common-query-unit" id="searchTitleDiv">
                <div class="form-group">
                    <div style="line-height: 25px;display: inline-block;padding-left:15px;">审核状态：</div>
                    <div style="width: 65%;display: inline-block;">
                        <select class="js-select2" name="checkStateInput" style="width:90%;margin-top: -2px;"
                                data-placeholder="">
                            <option value="">请选择：</option>
                            <c:forEach items="${checkStateList}" var="check">
                                <c:if test="${check == 0 && check == checkStateInput}">
                                    <option value="${check}" selected="selected">未对账</option>
                                </c:if>
                                <c:if test="${check == 0 && check != checkStateInput}">
                                    <option value="${check}" >未对账</option>
                                </c:if>

                                <c:if test="${check == 1 && check == checkStateInput}">
                                    <option value="${check}" selected="selected">已提交对账</option>
                                </c:if>
                                <c:if test="${check == 1 && check != checkStateInput}">
                                    <option value="${check}" >已提交对账</option>
                                </c:if>

                                <c:if test="${check == 2 && check == checkStateInput}">
                                    <option value="${check}" selected="selected">材料部已审核</option>
                                </c:if>
                                <c:if test="${check == 2 && check != checkStateInput}">
                                    <option value="${check}" >材料部已审核</option>
                                </c:if>

                                <c:if test="${check == 3 && check == checkStateInput}">
                                    <option value="${check}" selected="selected">对账完成</option>
                                </c:if>
                                <c:if test="${check == 3 && check != checkStateInput}">
                                    <option value="${check}" >对账完成</option>
                                </c:if>
                            </c:forEach>
            ...
            <div class="common-query-unit">
                <div class="form-group">
                    <div class="col-xs-12">
                        <button class="btn btn-minw btn-rounded btn-default" type="button" style="margin-top: -2px;"
                                data-toggle="tooltip" title="批量操作" onclick="submitCheck()">批量提交对账
                        </button>
                    </div>
                </div>
            </div>
    ...
    <table class="table table-bordered table-striped js-dataTable-simple dataTable no-footer" id="DataTables_Table_1"
        ...
        <tr role="row">
            <th class="sorting_disabled" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1"
                aria-sort="ascending" aria-label=": activate to sort column descending" style="width: 100px;">
            ...
            <th class="sorting_disabled" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1"
                aria-sort="ascending" aria-label=": activate to sort column descending" style="width: 100px;">审核状态
            </th>
            ...
            <th class="sorting_disabled" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1"
                aria-sort="ascending" aria-label=": activate to sort column descending" style="width: 100px;">材料部退回原因
            </th>
            <th class="sorting_disabled" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1"
                aria-sort="ascending" aria-label=": activate to sort column descending" style="width: 100px;">财务部退回原因
            </th>
                                        ...
                                        <c:forEach items="${checkState }" var="check">
                                            <c:if test="${order.checkState == check }">
                                                <button class="btn btn-xs btn-default" type="button" title="审核成功"
                                                        onclick="checkExamine(${order.id})">通过
                                                </button>
                                                <button class="btn btn-xs btn-default" type="button" title="退回"
                                                        onclick="back(${order.id})">驳回
                                                </button>
                                            </c:if>
                                        </c:forEach>
                                    </c:when>
...
<script type="text/javascript">
    ...
    function goList(page) {
        ...
        goMenu("/admin/order/orderListCheck", page, $("#searchForm").serialize());
    }

    function exportExcel() {
        var exportFormObj = document.getElementById("exportForm");
        exportFormObj.action = "/mxh/admin/order/exportOrderCheckList?type=3&" + $("#searchForm").serialize();
    ...
    function orderRemark(orderId) {
        layer.prompt({title: '请填写备注(不能为空)', formType: 2}, function(remarks, index){
            layer.close(index);
            index = layer.load(1, {
                shade: [0.5,'#000'] //0.1透明度的白色背景
            });
            console.log("提交备注:"+remarks)
            ajax({
                type: "POST",
                dataType: "html",
